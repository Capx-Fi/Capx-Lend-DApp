...
export const fetchHealthFactor = async (masterContract, loanID) => {
  let result = null;
  try {
    result = await masterContract.methods.healthFactor(loanID).call();
    if (result) {
...
export const fetchWVTAmt = async (
  masterContract,
  scAmt,
  wvtAddress,
  scAddress,
...
export const fetchPenalty = async (masterContract) => {
  let result = null;
  try {
    result = await masterContract.methods.penalty().call();
    if (result) {
      return result.toString(10);
    }
  } catch (error) {
